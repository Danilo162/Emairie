...
public function service(Request $request){
   ...
   $target = ($request->target)?$request->target:"";
    ...
    $data = self::getMairieId($id);
    $service = Service::findOrFail($target);
    ...
    return view("frontend.mairie.demande",compact('data','service','others','modes'));
}
...
public static function getMairieId($id){
    ...
    public function demande_store(Request $request){
        ...
        $mode = "account";
        if($request->mode_paiement!= 4){
            $mode =  $request->mode_paiement;
        }

  return  storeDemande(auth()->user()->administred_id,$request->mairie,$request->demande,$request->quantite,$mode,auth()->user()->id);
